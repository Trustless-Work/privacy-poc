...
- **witness** — per-circuit input builders (register / withdraw / transfer / set-spender / full-release delegated transfer), mirroring each circuit's public-input order.
...
- `test/escrow-witness.mjs` — confidential allowance funding, delegation-key recovery, and release-all witness invariants.
- `test/full-release-circuit.mjs` — executes the real funding and release circuits and proves that an otherwise-consistent partial release is rejected.
