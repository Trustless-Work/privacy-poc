...
const fieldPairVal = (a: bigint, b: bigint): xdr.ScVal => {
  const bytes = new Uint8Array(64);
  bytes.set(toBytes32BE(a), 0);
  bytes.set(toBytes32BE(b), 32);
  return scvBytes(bytes);
};
...
export function encodeSetSpenderData(w: SetSpenderWitness, proof: Uint8Array): xdr.ScVal {
  const p = w.payload;
  const payload = scvStruct({
    c_spend_new: pointVal(p.cSpendNew),
    c_a: pointVal(p.cA),
    escrowed_dvk: fieldPairVal(p.escrowedDvk.rX, p.escrowedDvk.cipher),
    b_tilde: fieldVal(p.bTilde),
    a_tilde: fieldVal(p.aTilde),
    r_e: pointVal(p.rE),
    sigma: fieldVal(p.sigma),
    sigma_a: fieldVal(p.sigmaA),
    v_aud_s: fieldVal(p.vAudS),
    b_aud_s: fieldVal(p.bAudS),
  });
  return envelope(payload, proof);
...
export function encodeFullReleaseData(w: FullReleaseWitness, proof: Uint8Array): xdr.ScVal {
  const p = w.payload;
  const payload = scvStruct({
    c_a_new: pointVal(p.cANew),
    c_tx: pointVal(p.cTx),
    r_e: pointVal(p.rE),
    v_tilde: fieldVal(p.vTilde),
    a_tilde_new: fieldVal(p.aTildeNew),
    sigma_a_new: fieldVal(p.sigmaANew),
    v_aud_r: fieldVal(p.vAudR),
    r_aud_r: fieldVal(p.rAudR),
    v_aud_s: fieldVal(p.vAudS),
    a_aud_s: fieldVal(p.aAudS),
  });
  return envelope(payload, proof);
}
