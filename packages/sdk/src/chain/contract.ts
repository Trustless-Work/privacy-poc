...
import type { SetSpenderWitness } from "../witness/set-spender.js";
import type { FullReleaseWitness } from "../witness/spender-transfer.js";
...
/** Initialize the one-milestone escrow and register its confidential account. */
export function submitInitializeEscrow(
  client: ChainClient,
  signer: Signer,
  ...
  approver: string,
  auditorId: number,
  registrationWitness: RegisterWitness,
  ...
  return client.invoke(
    ...
    [
      ...
      addr(receiver),
      addr(approver),
      addr(client.cfg.contracts.token),
      xdr.ScVal.scvU32(auditorId),
      encodeRegisterData(registrationWitness, registrationProof),
    ],
    signer,
  );
}

/** Fund the escrow by creating its complete private delegated allowance. */
export function submitFundEscrow(
  client: ChainClient,
  signer: Signer,
  escrowContract: string,
  ...
  return client.invoke(
    escrowContract,
    "fund",
    [xdr.ScVal.scvU32(liveUntilLedger), encodeSetSpenderData(witness, proof)],
    signer,
  );
}

/** One approver action releases the complete confidential allowance. */
export function submitApproveAndRelease(
  client: ChainClient,
  signer: Signer,
  escrowContract: string,
  witness: FullReleaseWitness,
  proof: Uint8Array,
): Promise<InvokeResult> {
  return client.invoke(
    escrowContract,
    "approve_and_release",
    [encodeFullReleaseData(witness, proof)],
    signer,
  );
}
