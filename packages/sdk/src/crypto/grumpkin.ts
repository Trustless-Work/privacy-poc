...
/**
 * Recover either valid affine point for an x-coordinate. Grumpkin ECDH uses
 * only the shared point's x-coordinate, so both square roots are equivalent
 * for the protocol's compact `escrowed_dvk` handoff.
 */
export function pointFromX(x: bigint): Point {
