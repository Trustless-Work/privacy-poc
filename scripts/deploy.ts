...
const TESTNET_USDC_ASSET = `USDC:${TESTNET_USDC_ISSUER}`;
...
const VK_FILES: ReadonlyArray<[string, number]> = [
  ...
  // This PoC registers the constrained release-all circuit in the standard
  // SpenderTransfer slot. Partial delegated transfers therefore cannot verify.
  ["spender_transfer_full_release", CIRCUIT_TYPE.SpenderTransfer],
...
async function main(): Promise<void> {
  ...
  // 1. Official testnet USDC SAC. XLM remains necessary only for fees.
  stellarSoft([
  ...
  if (underlying !== TESTNET_USDC_SAC) {
    throw new Error(`testnet USDC SAC mismatch: expected ${TESTNET_USDC_SAC}, got ${underlying}`);
  }
  console.log(`underlying (testnet USDC SAC) = ${underlying}`);
